  ...
  if (strpos($contentType, "application/json") !== false) {
    ...
    if (isset($data->name) && isset($data->date_of_birth) && isset($data->addrees) && isset($data->phone_number)) {
        ...
        if ($stmt->rowCount() > 0) {
            ...
            $id = $pdo->lastInsertId();
            $newStudent = $pdo->prepare("SELECT * FROM Management_School WHERE id = ?");
            $newStudent->execute([$id]);
            $printStudent = $newStudent->fetch(PDO::FETCH_ASSOC);
            echo json_encode([
                "message" => "Data added successfully",
                 "student_data" => $printStudent
            ]);
    ...
    if (!empty($name) && !empty($date_of_birth) && !empty($addrees) && !empty($phone_number)) {
        ...
        if ($stmt->rowCount() > 0) {
            ...
            $newStudent = $pdo->prepare("SELECT * FROM Management_School WHERE id = ?");
            $newStudent->execute([$id]);
            $printStudent = $printStudent->fetch(PDO::FETCH_ASSOC);
            echo json_encode([
                "message" => "Data added successfully",
                 "student_data" => $printStudent
            ]);
...
elseif($requestMethod == 'PUT'){
    ...
    if($id){
        ...
        if (isset($data->name)  && isset($data->date_of_birth) && isset($data->addrees) && isset($data->phone_number)){
            ...
            if ($stm->rowCount()> 0){
                $checkStmt = $pdo->prepare("SELECT * FROM Management_School WHERE id = ?");
                $checkStmt->execute([$id]);
                $printStudent = $checkStmt->fetch(PDO::FETCH_ASSOC);
                echo json_encode([
